            ...
            public void run() {
                try {
                    ...
                    if (responseCode / 100 == 4 || responseCode / 100 == 5) {
                        if (urlString.equals("http://sickinger-solutions.at/notesserver/todolists.php?")) {
                            responses[0] = new Response(httpURLConnection.getErrorStream(), responseCode);
                            Log.println(Log.DEBUG, "InternetConnectionHandler.java,SignIn", httpURLConnection.getResponseMessage());
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    if (responseCode / 100 == 4 || responseCode / 100 == 5) {
                        responses[0] = new Response(httpURLConnection.getErrorStream(), responseCode);
                        Log.println(Log.ERROR,"InternetConnectionHandler",httpURLConnection.getResponseMessage() + " " + httpURLConnection.getResponseCode());
                    } else {
                        responses[0] = new Response(httpURLConnection.getInputStream(), responseCode);
                    }
